...
type CatalogSong = { id:string; title:string; artist:string; album:string; image:string|null; url:string|null; isrc:string|null; provider:'spotify'|'musicbrainz'; key:number|null; mode:'major'|'minor'|null; confidence:number|null; progression:string|null };
const STARTER_SONGS = [
...
export default function Home() {
  ...
  async function saveSong() {
    ...
    const supabase=getSupabase(); if(userId&&supabase){
      if(selectedCatalog) await supabase.from('song_keys').insert({spotify_id:selectedCatalog.provider==='spotify'?selectedCatalog.id:null,musicbrainz_id:selectedCatalog.provider==='musicbrainz'?selectedCatalog.id:null,isrc:selectedCatalog.isrc,title:selectedCatalog.title,artist:selectedCatalog.artist,album:selectedCatalog.album,key_root:guess.root,mode:guess.mode,progression:analysisInput,confidence:guess.confidence,source:'chord-analysis',created_by:userId});
  ...
  return <main id="top">
    ...
    <section className="song-finder"><div><div><p className="step-label">SEARCH THE CATALOG</p><h2>Find a song or bring your own.</h2></div><span className="catalog-source">{searchState==='searching'?'Searching…':catalogProvider}</span></div><div className="song-search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search any song and artist" aria-label="Search songs"/></div><div className="song-list catalog-results">{visibleSongs.slice(0,8).map((song,index)=>{const catalog='provider' in song;return <article key={`${song.title}-${index}`} className={catalog&&song.image?'has-art':''}>{catalog&&song.image&&<img src={song.image} alt=""/>}<button onClick={()=>catalog?chooseCatalogSong(song as CatalogSong):loadSong(song)}><b>{song.title}</b><span>{song.artist}</span>{catalog&&<small>{song.key!==null?`${prettyNote(song.key,true)} ${song.mode} · ${song.confidence}% confidence`:'Key not verified yet'}</small>}</button>{catalog&&song.url&&<a href={song.url} target="_blank" rel="noreferrer">Open source ↗</a>}{!catalog&&'spotify' in song&&<a href={song.spotify} target="_blank" rel="noreferrer">Open in Spotify ↗</a>}</article>})}{searchState==='done'&&!visibleSongs.length&&<p className="empty-state">No match yet. Try adding the artist name.</p>}</div></section>
